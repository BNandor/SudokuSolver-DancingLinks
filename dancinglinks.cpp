...
class Sudoku{
	...
	public:
	int solution_sum;
	Sudoku():solution_sum(0),solution_before(0){
		//yes, we are eager like that
		generate_choices();
		generate_positions();
		init();	
	}
	...
	bool checkIntegrity(){
		...
		for(int i=0;i<possibility_per_cell;i++){
			...
			for(int j=0;j<possibility_per_cell;j++){        
				if(solution_mat[i][j]==0)continue;
				stat[solution_mat[i][j]]++;
				if(stat[solution_mat[i][j]]>1){
					return false;
				}
			}                
		}
		...
		for(int i=0;i<possibility_per_cell;i++){
			memset((void*)&stat,0,sizeof(stat));
			for(int j=0;j<possibility_per_cell;j++){
				if(solution_mat[j][i]==0)continue;
				stat[solution_mat[j][i]]++;
				if(stat[solution_mat[j][i]]>1){
					return false;
				}
			}
		}
		...
		for(int i=0;i<possibility_per_cell;i++){		
			for(int j=0;j<possibility_per_cell;j++){
				...
				boxstat[(i/3)*3 + j/3][solution_mat[j][i]]++;
				if(boxstat[(i/3)*3 + j/3][solution_mat[j][i]]>1){			
					return false;
				}
			}
		}
...
int main(int argc, const char *argv[])
{
	...
	while(getline(file,puzzle)){
		ss<<puzzle;
		solver.read_and_solve(ss);
#ifdef integrity		
		if(!solver.checkIntegrity()){
			cout<<"not ok"<<endl;
...
#ifdef total
	cout<<"Total solutions:"<<solver.solution_sum<<endl;	
#endif	
